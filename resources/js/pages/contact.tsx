...
export default function Contact({
    ...
    const pageTitle =
        ...
        settings?.contact?.contact_page_title ||
        'Get in Touch';
    const pageSubtitle =
        pageContent.hero?.subtitle ||
        settings?.contact?.contact_page_subtitle ||
    ...
    const formTitle =
        ...
        settings?.contact?.contact_form_title ||
        'Send us a Message';
    const formSubtitle =
        pageContent.form?.subtitle ||
        settings?.contact?.contact_form_subtitle ||
        "Fill out the form below and we'll get back to you as soon as possible.";
    const hoursParsed = (() => {
        try {
            return pageContent.hours?.content
                ? JSON.parse(pageContent.hours.content)
                : null;
        } catch {
            return null;
        }
    ...
    const hoursWeekday =
        hoursParsed?.weekday ||
        settings?.contact?.contact_hours_weekday ||
        'Saturday - Thursday: 10AM - 8PM';
    const hoursWeekend =
        hoursParsed?.weekend ||
        settings?.contact?.contact_hours_weekend ||
        'Friday: Closed';
    const contactCards =
        (pageContent.cards?.items as Array<{
            icon?: string;
            title?: string;
            details?: string[];
        }>) || [];
    ...
    const ctaParsed = (() => {
        try {
            return pageContent.cta?.content
                ? JSON.parse(pageContent.cta.content)
                : {};
        } catch {
    ...
    const fallbackCards = [
        {
            ...
            details: [
                ...
                settings?.contact?.address_line2 ||
                    settings?.contact?.contact_address_line2 ||
                    '',
            ].filter(Boolean),
        },
        {
            ...
            details: [
                settings?.contact?.phone ||
                    settings?.contact?.contact_phone ||
                    '+880 1XXX-XXXXXX',
                settings?.contact?.phone_support ||
                    settings?.contact?.contact_phone_support ||
                    '',
            ].filter(Boolean),
        },
        {
            ...
            details: [
                settings?.contact?.email ||
                    settings?.contact?.contact_email ||
                    'info@example.com',
                settings?.contact?.email_support ||
                    settings?.contact?.contact_email_support ||
                    '',
            ].filter(Boolean),
        },
        {
            icon: Clock,
            title: 'Working Hours',
            details: [hoursWeekday, hoursWeekend],
        },
    ];
                    ...
                    <div className="space-y-6">
                        <Card className="overflow-hidden p-0 pt-6">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Visit our showroom to see our e-club
