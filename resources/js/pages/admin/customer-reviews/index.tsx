        ...
        <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((star) => {
                ...
                return (
                    ...
                    >
                        <StarComp
                            className={`h-5 w-5 ${isFilled ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                        />
                    </button>
                );
            })}
    ...
    const handleCreate = async (data: ReviewFormData) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast.error(
                    error.message || 'Failed to create customer review',
                );
            }
        } catch (error) {
    ...
    const handleEdit = async (data: ReviewFormData) => {
        ...
        try {
            ...
            const response = await fetch(
                `/api/admin/customer-reviews/${selectedReview.id}`,
                {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
            ...
            if (response.ok) {
                ...
                const error = await response.json();
                toast.error(
                    error.message || 'Failed to update customer review',
                );
            }
        } catch (error) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const error = await response.json();
                toast.error(
                    error.message || 'Failed to delete customer review',
                );
            }
        } catch (error) {
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input
                                            placeholder="e.g., Business Owner, Interior Designer"
                                            {...field}
                                            value={field.value || ''}
                                        />
                                    </FormControl>
                                    <FormDescription>
                                        Customer's profession or role
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-4">
                                {imagePreview ? (
                                    ...
                                    <div
                                        className="flex h-20 w-20 cursor-pointer items-center justify-center rounded-full border-2 border-dashed border-gray-300 transition-colors hover:border-primary"
                                        onClick={() =>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Optional. Square images work best (e.g.,
                                200x200px)
                            </p>
                        </div>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                            <div>
                                ...
                                <CardDescription>
                                    Manage customer testimonials displayed on
                                    your website
                                </CardDescription>
                            </div>
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                <DialogContent className="max-w-lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Add a new customer testimonial to display on your
                            website
                        </DialogDescription>
                    </DialogHeader>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            {selectedReview?.name}"? This action cannot be
                            undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
