...
function NavGroup({
    ...
    const handleCollapsedNavigation = (itemUrl: string, itemTitle: string) => {
        ...
        sessionStorage.setItem(
            SIDEBAR_LAST_ITEM_KEY,
            JSON.stringify({ groupKey, itemTitle }),
        );
    };
        ...
        >
            <SidebarGroup className="border-b border-sidebar-border last:border-b-0">
                <CollapsibleTrigger asChild>
                    <SidebarGroupLabel className="cursor-pointer rounded-md transition-colors hover:bg-sidebar-accent/50">
...
function SimpleNavGroup({
    ...
    const handleCollapsedNavigation = (itemTitle: string) => {
        if (isCollapsed) {
            ...
            sessionStorage.setItem(
                SIDEBAR_LAST_ITEM_KEY,
                JSON.stringify({ groupKey: 'settings', itemTitle }),
            );
        }
    };
                        ...
                        <SidebarMenuItem key={item.title}>
                            ...
                            >
                                <Link
                                    ...
                                    preserveState
                                    onClick={() =>
                                        handleCollapsedNavigation(item.title)
                                    }
                                >
                                    <item.icon className="shrink-0" />
                                    <span>{item.title}</span>
    ...
    return (
        <Sidebar collapsible="icon" {...props}>
                    ...
                    <SidebarMenuItem>
                        ...
                        >
                            <Link href="/admin" preserveScroll preserveState>
                                <div className="flex aspect-square h-full w-full shrink-0 items-center justify-center">
                                    <img
                                        src={logoSrc}
                                        alt={siteName}
            ...
            <SidebarContent className="overflow-hidden p-0">
                <ScrollArea
                    ref={scrollRef}
                    className="h-full p-0 [&>[data-radix-scroll-area-viewport]]:h-full"
